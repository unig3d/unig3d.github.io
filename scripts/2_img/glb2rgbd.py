...
def render_scene(output_path, fast_mode: bool):
    ...
    bpy.context.scene.render.image_settings.color_mode = "RGB" #"RGBA" #"BW"
...
def save_rendering_dataset(
    ...
    assert light_mode in ["random", "uniform", "camera", "area"]
    assert camera_pose in ["random", "z-circular", "z-circular-elevated"]


    import_model(input_path) #import 3D model with blender api
